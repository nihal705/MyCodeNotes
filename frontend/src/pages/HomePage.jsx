...
import { 
  FiArrowRight, FiBook, FiCode, FiTrendingUp, 
  FiGithub, FiLinkedin, FiAward,
...
const HomePage = () => {
  ...
  const fetchProblems = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      console.log(`✅ Total problems: ${problems.length}, Showing: ${Math.min(4, problems.length)} most recent`)
    } catch (error) {
    ...
    >
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-4">
              ...
              <a href={profile.leetcode} target="_blank" rel="noopener"
                 className="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors">
      ...
      >
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-beige-200 text-center">
          <div className="text-3xl font-bold text-leetcode-yellow">{stats.totalSolved}</div>
          <div className="text-sm text-gray-600">Problems Solved</div>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-beige-200 text-center">
          <div className="text-3xl font-bold text-leetcode-yellow">{stats.maxStreak}</div>
          <div className="text-sm text-gray-600">Max Streak</div>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-beige-200 text-center">
          <div className="text-3xl font-bold text-leetcode-yellow">{stats.activeDays}</div>
          <div className="text-sm text-gray-600">Active Days</div>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-beige-200 text-center">
          <div className="text-3xl font-bold text-leetcode-yellow">{stats.totalSubmissions}</div>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-bold text-gray-900">LeetCode Progress</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-green-50 rounded-xl p-4 text-center border border-green-200">
            ...
            <div className="text-xs text-gray-500">/ {stats.easy + stats.medium + stats.hard} total</div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              {Object.entries(stats.languages).map(([lang, count]) => (
                <span key={lang} className="text-sm bg-gray-100 px-3 py-1 rounded-full">
                  {lang} ({count})
                </span>
              ))}
            </div>
      ...
      >
        <Link to="/problems" className="group bg-white rounded-2xl p-6 shadow-sm border border-beige-200 hover:shadow-lg transition-all duration-300 hover:border-leetcode-yellow">
          <FiBook className="text-3xl text-leetcode-yellow mb-3" />
          <h3 className="text-xl font-semibold mb-2">LeetCode Problems</h3>
          <p className="text-gray-600 text-sm mb-3">Access all my solved problems with detailed notes</p>
        ...
        <Link to="/practice" className="group bg-white rounded-2xl p-6 shadow-sm border border-beige-200 hover:shadow-lg transition-all duration-300 hover:border-leetcode-yellow">
          <FiCode className="text-3xl text-leetcode-yellow mb-3" />
          <h3 className="text-xl font-semibold mb-2">Practice & Learn</h3>
          <p className="text-gray-600 text-sm mb-3">Practice coding and general programming problems with hints and hidden solutions</p>
          <span className="inline-flex items-center text-leetcode-yellow font-medium group-hover:translate-x-2 transition-transform">
            Start Practice <FiArrowRight className="ml-2" />
          </span>
        </Link>

        <Link to="/concepts" className="group bg-white rounded-2xl p-6 shadow-sm border border-beige-200 hover:shadow-lg transition-all duration-300 hover:border-leetcode-yellow">
          <FiTrendingUp className="text-3xl text-leetcode-yellow mb-3" />
          <h3 className="text-xl font-semibold mb-2">Programming Concepts</h3>
          <p className="text-gray-600 text-sm mb-3">Quick reference for programming concepts with examples</p>
          <span className="inline-flex items-center text-leetcode-yellow font-medium group-hover:translate-x-2 transition-transform">
      ...
      <motion.section variants={itemVariants}>
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <FiRefreshCw size={14} className={(refreshing || isFetchingRef.current) ? 'animate-spin' : ''} />
              {(refreshing || isFetchingRef.current) ? 'Refreshing...' : 'Refresh'}
            </button>
            <Link to="/problems" className="text-leetcode-yellow hover:underline">
              View All →
            </Link>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {recentProblems.length > 0 ? (
            ...
            <div className="col-span-2 text-center py-8 text-gray-500">
              No problems added yet. Go to Admin panel to add your first problem!
            </div>
          )}
        </div>
      </motion.section>
